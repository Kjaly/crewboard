...
**Состояние:** 0.3.0, первый публичный выпуск, в npm пока нет · Node.js 24+ · используется на macOS с кандидатами в релиз dsh 0.1.5 · Linux и Windows не проверялись
...
![Скриншот графа Crewboard: задачи одного плана на шести дорожках с их воркерами; в боковой панели другой репозиторий помечен «Ждут вас».](docs/assets/hero-graph.png)
...
3. **Сначала проверяет оркестратор.** Когда воркер закончил, оркестратор берёт результат на проверку (`crewboard verify`). Он передаёт задачу вам со сводкой или возвращает воркеру с замечаниями. Это включено, пока у плана есть чат; иначе законченная работа сразу приходит к вам.
...
Понадобятся Node.js 24 или новее, Git и pnpm 11.5.2 для сборки. Для экрана нужен dsh; для каждого воркера — его CLI, установленный и с выполненным входом (`claude`, `codex`, `devin`, `dsh`).

> [!IMPORTANT]
> Пакеты npm `crewboard` и `dsh-crewboard` появятся с выпуском 0.3.0. До тех пор устанавливайте из исходников.
...
1. Запустите `dsh web` и откройте вкладку **«Оркестрация»**.
