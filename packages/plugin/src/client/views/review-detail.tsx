...
const execution = (state?: string) => state === 'completed' || state === 'failed' || state === 'cancelled' || state === 'incomplete' || state === 'running' ? t(`drill.execution.${state}`) : t('review.unavailable')
...
function RunDetail({ repo, run, summary, task, beside, backTask, backPanel, onBack, onTask, onExpand }: { repo: RepoSnapshot; run: PlanRunCost; summary?: TaskReviewSummary; task: TaskReviewDetail | null; beside: boolean; backTask?: boolean; backPanel?: boolean; onBack(): void; onTask(): void; onExpand(): void }) {
  ...
  return <section className={`orc-drill ${beside ? 'orc-drill--beside' : 'orc-drill--main'}`} aria-label={t('review.runDetail')}>
    ...
    <h2 tabIndex={-1} ref={heading}>{run.taskTitle}</h2><p className="orc-drill__sub"><StatusWord status={status} cancelled={(run.executionOutcome ?? run.outcome) === 'cancelled'} incomplete={(run.executionOutcome ?? run.outcome) === 'incomplete'} /> · <code>{run.runId}</code></p>
