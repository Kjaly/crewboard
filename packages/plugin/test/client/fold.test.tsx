...
it('auto folds every finished lane of a plan larger than 24, whatever its size', () => {
  expect(decideFolds(repo([...finished('2F', 6), ...live(6)]), {}).folded.size).toBe(0)
  expect(decideFolds(repo([...finished('2F', 6), ...live(34)]), {}).folded.has('2F')).toBe(true)
  // nv1: a small finished lane is history too — it folds like a big one.
  expect(decideFolds(repo([...finished('2F', 1), ...live(37)]), {}).folded.has('2F')).toBe(true)
})

it('keeps live lanes open and folds finished ones, however fresh', () => {
  ...
  const decision = decideFolds(repo(tasks), {})
  expect(decision.folded.has('Идёт')).toBe(false)
  expect(decision.folded.has('Свежая')).toBe(true)
  expect(decision.folded.has('Готова')).toBe(true)
  expect(decision.folded.has('Работа')).toBe(false)
})

it('folds the Decisions band once every decision is closed, and not before', () => {
  const decisions = (status: 'accepted' | 'ready') => Array.from({ length: 3 }, (_, i) => makeTask({ id: `d${i + 1}`, kind: 'decision', lane: 'Работа', status: i === 0 ? status : 'accepted' }))
  expect(decideFolds(repo([...decisions('accepted'), ...live(30)]), {}).folded.has('__decision_lane__')).toBe(true)
  expect(decideFolds(repo([...decisions('ready'), ...live(30)]), {}).folded.has('__decision_lane__')).toBe(false)
})

it('a manual unfold still wins over the automatic fold of a finished lane', () => {
  const source = repo([...finished('2F', 2), ...live(30)])
  expect(decideFolds(source, { '2F': false }).folded.has('2F')).toBe(false)
  expect(decideFolds(source, { Работа: true }).folded.has('Работа')).toBe(true)
})
...
it('folding by hand hides the whole lane — guests included', () => {
  ...
  tasks[6] = { ...tasks[6]!, deps: ['Готова1'] }
  const source = repo(tasks)
  const decision = decideFolds(source, { Готова: true })
  expect(decision.folded.has('Готова')).toBe(true)
...
it('reserves nonintersecting rectangles for three folded lanes, guests, two open lanes and their labels', () => {
  ...
  const source = repo(tasks)
  const decision = decideFolds(source, { '1A': true, '2A': true, '3A': true })
  const graph = foldGraph(source, decision)
