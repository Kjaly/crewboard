...
describe('cost', () => {
  ...
  it('summarises per agent and keeps unknown money unknown', () => {
    const totals = summarizeCosts([
      ...
      { runId: 'c', agent: 'deepseek-flash', durationSec: 10, usd: 0.02 },
    ])
    expect(totals.devin).toEqual({ runs: 2, durationSec: 150 })
    expect(totals['deepseek-flash']).toEqual({ runs: 1, durationSec: 10, usd: 0.02 })
  })
  ...
  it('prefers backend usage for money and tokens', () => {
    ...
    expect(c).toMatchObject({ runId: 'run_dsh-a', agent: 'dsh/deepseek-flash', durationSec: 9, usd: 0.002, cashUsd: { value: 0.002, currency: 'USD' }, calls: 5, tokens: { input: 120, output: 20, cacheRead: 100, reasoning: 10 }, availability: { input: 'known', cash: 'known' } })
    ...
    expect(pending.usd).toBeUndefined()
  ...
  it('sums tokens and counts pending runs per agent', () => {
    const totals = summarizeCosts([
      { runId: 'a', agent: 'dsh', durationSec: 9, usd: 0.002, calls: 5, tokens: { input: 100, output: 10, cacheRead: 50, reasoning: 5 } },
      { runId: 'b', agent: 'dsh', durationSec: 3, pending: true },
    ])
    expect(totals.dsh).toEqual({ runs: 2, durationSec: 12, usd: 0.002, tokens: { input: 100, output: 10, cacheRead: 50, reasoning: 5 }, pendingRuns: 1 })
  })
