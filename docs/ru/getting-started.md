...
- Node.js 24 или новее, Git и `npm install -g crewboard`.
...
crewboard attention        # что ждёт вас: проверки, решения, упавшие запуски
...
Каждая команда просит подтверждения и отказывается работать без интерактивного терминала — агент не может принять собственную работу. После приёмки рабочая копия удаляется, если так настроена очистка. Вердикт и доказательства описаны в разделе [приёмка](review.md).

## Экран

Когда [плагин](plugin-setup.md) установлен, откройте **Оркестрация** в боковой панели dsh; там же видно, сколько задач ждут вас. Экран показывает те же планы, что и CLI. Настройки — в разделе **Crewboard** настроек dsh.
